...
all: | $(BIN) ; $(info $(M) building executable to $(BUILDPATH)) @ ## Build program binary
	$Q $(GO) build \
		...
		-o $(BUILDPATH) cmd/demo/main.go

.PHONY: docker_build
docker_build: ; $(info $(M) building docker image) @ ## Build docker image
	docker build -t middlewaregruppen/container-demo:${VERSION} .
	docker tag middlewaregruppen/container-demo:${VERSION} middlewaregruppen/container-demo:latest
